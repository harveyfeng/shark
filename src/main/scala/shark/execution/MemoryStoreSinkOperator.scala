...
class MemoryStoreSinkOperator extends TerminalOperator {
  ...
  override def execute(): RDD[_] = {
    ...
    if (tachyonWriter != null) {
      ...
      if (cacheMode == CacheType.MEMORY) outputRDD.persist(StorageLevel.MEMORY_AND_DISK)
      outputRDD.context.runJob(
        outputRDD, (iter: Iterator[TablePartition]) => iter.foreach(_ => Unit))
    }

    // Put the table in Spark block manager or Tachyon.
    op.logInfo("Putting %sRDD for %s.%s in %s storage".format(
      if (useUnionRDD) "Union" else "",
      databaseName,
      tableName,
      if (cacheMode == CacheType.NONE) "no managed" else cacheMode.toString))
    ...
    val tableStats = if (isHivePartitioned) {
      ...
      partitionedTable.getStats(hivePartitionKey)
    } else {
